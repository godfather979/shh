...
const ApnaDocSummarizer = () => {
  ...
  const mockResponse = {
    ...
    "belongs": "This document appears to relate to **Section 420 (Cheating and dishonestly inducing delivery of property)** and **Section 406 (Punishment for criminal breach of trust)** of the Indian Penal Code (IPC)."
  };
    ...
    return (
        <div className="w-full">
            <div className="relative pt-1">
                <div className="flex mb-2 items-center justify-between">
                    ...
                    <div className="text-right">
                        <span className={`text-xs font-semibold inline-block ${percentage > 66 ? 'text-green-600' : percentage > 33 ? 'text-yellow-600' : 'text-red-600'}`}>
                            {percentage}%
                        </span>
                    </div>
                </div>
            ...
            <p className="text-center text-sm text-slate-600 mt-1">
                Estimated probability the legal outcome will be favourable based on the document's clauses.
            </p>
        </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
      <div className=" mx-auto px-4 py-12">
        ...
        {responseData && !isLoading && !showSummary && ( // Show button only if data loaded and summary not yet shown
          <motion.div
        ...
        {isLoading && (
          ...
          >
            <div className="flex flex-col items-center">
              ...
              <p className="mt-4 text-slate-600">Analyzing document...</p>
            </div>
          </motion.div>
        )}
        ...
        {showSummary && responseData && !isLoading && (
          ...
          >
            <div className="flex flex-wrap -mx-4 h-4/5 min-h-screen"> {/* Updated height to 4/5 of screen height */}
              {/* Section 1: Parsed Clauses Table */}
                {responseData.parsed_clauses && (
                  <motion.div
                    ...
                    variants={fadeInUp}
                  >
                    <div className="bg-white rounded-xl shadow-lg p-6 border border-slate-200 h-full overflow-auto max-h-[100vh]"> {/* Added max-h-[60vh] for fixed height and overflow-auto for scrolling */}
              ...
              {responseData.gdpr_compliance_report && (
                <motion.div
                  ...
                  variants={fadeInUp}
                >
                  <div className="bg-white rounded-xl shadow-lg p-6 border border-slate-200 h-full overflow-auto mt-8 md:mt-0 max-h-[100vh]"> {/* Added h-full and margin-top on mobile */}
                    <h2 className="text-xl font-bold text-slate-800 mb-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-transparent bg-clip-text top-0 bg-white pb-2">
                      GDPR Compliance Report
                    </h2>
                    {/* Render markdown text using ReactMarkdown */}
            ...
            {/* Section 3: Probability Graph */}
            {typeof responseData.probability === 'number' && // Check if probability exists and is a number
              !isNaN(responseData.probability) && // Check if it's not NaN
              responseData.probability >= 0 && responseData.probability <= 1 && ( // Check if within valid range
                <motion.div
                  className="bg-white rounded-xl shadow-lg p-6 border border-slate-200 mt-10" 
                  variants={fadeInUp}
                >
                  <h2 className="text-2xl font-bold text-slate-800 mb-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-transparent bg-clip-text">
                    Outcome Probability
                  </h2>
                  {renderProbabilityGraph(responseData.probability)}
                </motion.div>
              )}


            {/* Section 4: Belongs Section */}
